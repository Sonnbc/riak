...
extract_read_delay(Options) ->
    if is_list(Options) ->
        case Options of
            [] -> false;
            [X | Xs] -> 
                case X of
                    {read_delay, 0} -> false; %avoid timer:sleep(0)
                    {read_delay, V} -> V;
                    _ -> extract_read_delay(Xs)
                end
        end;
    true -> false
    end.
        
%real get function that handles all the work
get_worker(Bucket, Key, Options) ->
    Me = self(),
    ReqId = mk_reqid(),
    case node() of
        Node ->
...
get(Bucket, Key, Options) when is_list(Options) ->
    ...
    if is_integer(D) -> 
        timer:sleep(D),
        get_worker(Bucket, Key, Options);
    true -> get_worker(Bucket, Key, Options)            
    end;
